...
struct FuelCameraView: View {
    ...
    nonisolated enum ScanMode: String, CaseIterable {
    ...
    private func cornerShape(at point: CGPoint, corner: Corner, length: CGFloat, lineWidth: CGFloat) -> some View {
        Path { path in
            switch corner {
            case .topLeft:
        ...
        .stroke(brandOrange, style: StrokeStyle(lineWidth: lineWidth, lineCap: .round))
    }
        ...
        VStack(spacing: 20) {
            ...
            HStack(spacing: 0) {
                ...
                Button {
                    ...
                    ZStack {
                        Circle()
                            .strokeBorder(brandOrange, lineWidth: 4)
        ...
        HStack(spacing: 6) {
            ForEach(ScanMode.allCases, id: \.self) { mode in
                Button {
                    ...
                    .foregroundStyle(scanMode == mode ? .black : .white.opacity(0.7))
...
struct FuelCameraPreview: UIViewControllerRepresentable {
    ...
    class Coordinator: NSObject {
        ...
        init(_ parent: FuelCameraPreview) {
            self.parent = parent
            super.init()
            NotificationCenter.default.addObserver(self, selector: #selector(handleCapture), name: .fuelCameraCapture, object: nil)
        }
        ...
        @objc nonisolated func handleCapture() {
            Task { @MainActor in
                parent.onCapture()
            }
        }
