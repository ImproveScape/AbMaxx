...
struct BarcodeScannerView: View {
        ...
        VStack(spacing: 20) {
            ...
            ZStack {
                ...
                Image(systemName: "barcode")
                    ...
                    .foregroundStyle(brandRed.opacity(0.5))
            }
    ...
    private func resultState(_ result: NutritionLookupResult) -> some View {
        VStack(spacing: 0) {
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HStack(spacing: 12) {
                            macroChip("Protein", value: result.protein, color: .blue)
                            macroChip("Carbs", value: result.carbs, color: brandOrange)
                            macroChip("Fat", value: result.fat, color: .purple)
                        }
                    }
                    .padding(18)
                    .background(Color(white: 0.1))
            ...
            Button {
                viewModel.addFoodFromAIResult(result, mealType: mealType)
                dismiss()
            } label: {
                HStack(spacing: 8) {
...
class BarcodeCameraViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    private func setupCamera() {
        ...
        scanFrame.layer.borderColor = UIColor.systemRed.cgColor
        scanFrame.layer.borderWidth = 2.5
