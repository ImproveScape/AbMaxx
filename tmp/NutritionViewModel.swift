...
class NutritionViewModel {
    var todayNutrition: DailyNutrition = DailyNutrition()
    var weeklyHistory: [DailyNutrition] = []
    ...
    func addFoodFromAIResult(_ result: NutritionLookupResult, mealType: MealType) {
        let entry = FoodEntry(
            name: result.name,
    ...
    func analyzeFoodImageInline(_ imageData: Data, mealType: MealType) async {
        ...
        if results.isEmpty {
            ...
            for result in results {
                addFoodFromAIResult(result, mealType: mealType)
            }
