...
class LeaderboardService {
    ...
    private var supabaseURL: String { Config.allValues["EXPO_PUBLIC_MY_SUPABASE_URL"] ?? Config.allValues["EXPO_PUBLIC_SUPABASE_URL"] ?? "" }
    private var supabaseKey: String { Config.allValues["EXPO_PUBLIC_MY_SUPABASE_ANON_KEY"] ?? Config.allValues["EXPO_PUBLIC_SUPABASE_ANON_KEY"] ?? "" }
    ...
    func fetchLeaderboard(limit: Int = 50) async -> [LeaderboardMember] {
        guard isConfigured else { return [] }
        guard let url = URL(string: "\(supabaseURL)/rest/v1/leaderboard?select=*&order=score.desc&limit=\(limit)") else { return [] }
