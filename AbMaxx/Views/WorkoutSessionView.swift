...
struct WorkoutSessionView: View {
    ...
    private let accentBright = AppTheme.primaryAccent.opacity(0.85)
    ...
    private var mainSessionContent: some View {
        ZStack {
            if !parsed.isTimeBased && sessionPhase == .active {
                cameraActiveView
            } else if !parsed.isTimeBased && sessionPhase == .countdown {
                cameraCountdownView
            } else {
                    ...
                    .stroke(
                        LinearGradient(
                            colors: targetReached
                                ...
                                : [AppTheme.primaryAccent, AppTheme.primaryAccent.opacity(0.7)],
    ...
    private var exerciseImageSection: some View {
        VStack(spacing: 0) {
            if let ex = currentExercise {
                ExerciseFormGuideView(exercise: ex, showFullGuide: true)
            }
        }
    ...
    private var guideActions: some View {
        VStack(spacing: 10) {
            if let tip = coachTipForGuide {
                HStack(spacing: 8) {
                    Image(systemName: !parsed.isTimeBased ? "camera.fill" : "hand.point.up.fill")
                        .font(.system(size: 11, weight: .bold))
                        .foregroundStyle(AppTheme.primaryAccent.opacity(0.8))
                    Text(!parsed.isTimeBased ? "Camera will count your reps" : tip)
            ...
            HStack(spacing: 8) {
                Button {
                    ...
                    HStack(spacing: 8) {
                        Image(systemName: !parsed.isTimeBased ? "camera.fill" : "play.fill")
                            .font(.system(size: 16, weight: .bold))
                        Text("SET \(currentSet)")
                    ...
                    .background(
                        !parsed.isTimeBased
                            ? LinearGradient(colors: [AppTheme.primaryAccent, AppTheme.primaryAccent.opacity(0.8)], startPoint: .leading, endPoint: .trailing)
                            : LinearGradient(colors: [AppTheme.primaryAccent, AppTheme.primaryAccent.opacity(0.8)], startPoint: .leading, endPoint: .trailing)
                    )
                    .clipShape(.rect(cornerRadius: 16))
