...
struct FuelCameraView: View {
    ...
    private var simulatorPlaceholder: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                Text("Camera is not available.\nUse the photo library button below\nto select a meal photo.")
    ...
    private var topBar: some View {
        HStack {
            ...
            Spacer()
            Text("AbMaxx").font(.system(size: 22, weight: .bold)).foregroundStyle(.white)
            Spacer()
    ...
    private var bottomControls: some View {
        VStack(spacing: 20) {
            HStack(spacing: 0) {
                ...
                Button {
                    #if targetEnvironment(simulator)
                    // No camera on simulator — guide user to photo library
                    #else
                    capturePhoto()
                    #endif
                } label: {
