...
nonisolated enum AnthropicError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        ...
        case .httpError(let status, let body): return "Anthropic API error \(status): \(body)"
        case .invalidResponse: return "Invalid response from Anthropic API"
        case .emptyResponse: return "Empty response from Anthropic API"
        }
    }
}
...
class AnthropicService {
    ...
    func chatWithVision(
        ...
        let data = try await sendRequest(body: body, timeout: 120)
        return try extractText(from: data)
    }

    private func sendRequest(body: [String: Any], timeout: TimeInterval = 120) async throws -> Data {
