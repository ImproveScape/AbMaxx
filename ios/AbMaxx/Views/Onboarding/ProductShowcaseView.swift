...
private struct ShowcaseConfig {
    ...
    static let aiScanner = ShowcaseConfig(
        category: "AI ABS SCAN",
        icon: "viewfinder",
        title: "See What The\nMirror Can't",
        tagline: "Every ab muscle. Analyzed.",
        description: "Our AI breaks down your upper abs, lower abs, obliques, and deep core individually — exposing exactly which muscles are holding you back from visible abs.",
        accentColor: AppTheme.primaryAccent,
        mockContent: AnyView(ScannerMockScreen())
    )

    static let personalizedTraining = ShowcaseConfig(
        category: "YOUR ABS PLAN",
        icon: "figure.core.training",
        title: "Your Weak Zones\nBecome Strong",
        tagline: "Every rep targets what you actually need.",
        description: "Your scan tells us exactly which ab regions are underdeveloped. Your plan attacks those zones first — so you stop wasting time on exercises that aren't moving the needle.",
        accentColor: AppTheme.primaryAccent,
        mockContent: AnyView(TrainingMockScreen())
    )

    static let progressTracking = ShowcaseConfig(
        category: "ABS PROGRESS",
        icon: "chart.line.uptrend.xyaxis",
        title: "Watch Each\nAb Zone Grow",
        tagline: "Real scores. Real progress. Weekly.",
        description: "Every weekly scan tracks how each part of your abs is developing. You'll see exactly which zones are responding and which need more work — no more guessing.",
        accentColor: AppTheme.success,
        mockContent: AnyView(ProgressMockScreen())
    )

    static let nutritionTracking = ShowcaseConfig(
        category: "REVEAL YOUR ABS",
        icon: "flame.fill",
        title: "Strip The Fat\nShow The Muscle",
        tagline: "Your abs are there — diet reveals them.",
        description: "You can have the strongest abs in the world and never see them. Hit your macros with AI meal tracking and watch the layer hiding your abs disappear week by week.",
...
private struct NutritionMockScreen: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            ...
            foodRow(name: "Grilled Chicken Breast", cal: "284", time: "12:30 PM", icon: "🍗")
            foodRow(name: "Greek Yogurt + Berries", cal: "180", time: "9:15 AM", icon: "🫐")
            foodRow(name: "Protein Shake", cal: "220", time: "7:00 AM", icon: "🥤")
