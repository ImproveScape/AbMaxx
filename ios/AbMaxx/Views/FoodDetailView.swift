...
struct FoodDetailView: View {
    ...
    private var vitaminsCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("Vitamins").font(.system(size: 17, weight: .semibold)).foregroundStyle(AppTheme.primaryText)
            VStack(spacing: 0) {
                microRow(icon: "sun.max.fill", label: "Vitamin A", value: entry.vitaminA * entry.quantity, unit: "mcg", dailyValue: 900, color: Color(hex: "FF9F0A"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "bolt.circle.fill", label: "Vitamin C", value: entry.vitaminC * entry.quantity, unit: "mg", dailyValue: 90, color: Color(hex: "FF9F0A"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "sun.min.fill", label: "Vitamin D", value: entry.vitaminD * entry.quantity, unit: "mcg", dailyValue: 20, color: Color(hex: "FFD60A"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "shield.fill", label: "Vitamin E", value: entry.vitaminE * entry.quantity, unit: "mg", dailyValue: 15, color: AppTheme.success)
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "leaf.circle.fill", label: "Vitamin K", value: entry.vitaminK * entry.quantity, unit: "mcg", dailyValue: 120, color: AppTheme.success)
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "figure.walk", label: "Vitamin B6", value: entry.vitaminB6 * entry.quantity, unit: "mg", dailyValue: 1.7, color: Color(hex: "BF5AF2"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "drop.circle.fill", label: "Vitamin B12", value: entry.vitaminB12 * entry.quantity, unit: "mcg", dailyValue: 2.4, color: Color(hex: "FF375F"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "staroflife.fill", label: "Folate", value: entry.folate * entry.quantity, unit: "mcg", dailyValue: 400, color: AppTheme.success)
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "circle.grid.cross.fill", label: "Thiamin (B1)", value: entry.thiamin * entry.quantity, unit: "mg", dailyValue: 1.2, color: Color(hex: "64D2FF"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "circle.grid.2x2.fill", label: "Riboflavin (B2)", value: entry.riboflavin * entry.quantity, unit: "mg", dailyValue: 1.3, color: Color(hex: "64D2FF"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "circle.grid.3x3.fill", label: "Niacin (B3)", value: entry.niacin * entry.quantity, unit: "mg", dailyValue: 16, color: Color(hex: "64D2FF"))
            }
        }
        .padding(18)
        .cardStyle()
    ...
    private var mineralsCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("Minerals").font(.system(size: 17, weight: .semibold)).foregroundStyle(AppTheme.primaryText)
            VStack(spacing: 0) {
                microRow(icon: "circle.fill", label: "Calcium", value: entry.calcium * entry.quantity, unit: "mg", dailyValue: 1000, color: Color(hex: "0066FF"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "flame.fill", label: "Iron", value: entry.iron * entry.quantity, unit: "mg", dailyValue: 18, color: Color(hex: "FF6B6B"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "sparkle", label: "Magnesium", value: entry.magnesium * entry.quantity, unit: "mg", dailyValue: 420, color: Color(hex: "64D2FF"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "hexagon.fill", label: "Zinc", value: entry.zinc * entry.quantity, unit: "mg", dailyValue: 11, color: Color(hex: "AC8E68"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "atom", label: "Phosphorus", value: entry.phosphorus * entry.quantity, unit: "mg", dailyValue: 1250, color: .teal)
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "seal.fill", label: "Selenium", value: entry.selenium * entry.quantity, unit: "mcg", dailyValue: 55, color: Color(hex: "FFD60A"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "diamond.fill", label: "Copper", value: entry.copper * entry.quantity, unit: "mg", dailyValue: 0.9, color: Color(hex: "AC8E68"))
                Divider().overlay(AppTheme.border).padding(.leading, 40)
                microRow(icon: "triangle.fill", label: "Manganese", value: entry.manganese * entry.quantity, unit: "mg", dailyValue: 2.3, color: Color(hex: "BF5AF2"))
            }
        }
        .padding(18)
        .cardStyle()
    ...
    private func enrichEntry() async {
        let hasMicros = entry.fiber > 0 || entry.sugar > 0 || entry.sodium > 0 || entry.calcium > 0 || entry.iron > 0 || entry.potassium > 0
        guard !hasMicros else {
            isEnriching = false
            return
        }
        isEnriching = true
        ...
        withAnimation(.easeInOut(duration: 0.3)) {
            ...
            entry.vitaminD = result.vitaminD ?? 0; entry.vitaminE = result.vitaminE ?? 0; entry.vitaminK = result.vitaminK ?? 0
            entry.vitaminB6 = result.vitaminB6 ?? 0; entry.vitaminB12 = result.vitaminB12 ?? 0; entry.folate = result.folate ?? 0
            entry.magnesium = result.magnesium ?? 0; entry.zinc = result.zinc ?? 0; entry.phosphorus = result.phosphorus ?? 0
            entry.thiamin = result.thiamin ?? 0; entry.riboflavin = result.riboflavin ?? 0; entry.niacin = result.niacin ?? 0
            entry.manganese = result.manganese ?? 0; entry.selenium = result.selenium ?? 0; entry.copper = result.copper ?? 0
        }
